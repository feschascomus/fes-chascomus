...
export default function AdminFesPage({ params }) {
  ...
  useEffect(() => {
    const iniciar = async () => {
      ...
      await cargarTodo(codigo)
    }

    iniciar()
  }, [params])

  const cargarTodo = async (codigo) => {
    setCargando(true)
    setError("")
    ...
    return (
      <main className="min-h-screen bg-slate-100 p-4 sm:p-6">
        <div className="max-w-4xl mx-auto space-y-4">
          <Link
            href={usuario ? `/panel/${usuario.codigo}` : "/login"}
    ...
    <main className="min-h-screen bg-slate-100 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <Link
            href={`/publicar-novedad/${codigoUsuario}`}
          ...
          <Link
            href={`/publicar-cuadernillo/${codigoUsuario}`}
            className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl transition hover:shadow-2xl"
          >
            <h2 className="text-2xl font-bold text-slate-900">
